...
Module: Arrays
Assignment: M2B-22
Program: 263c Naive Prime Optimization
*/
/** REQUIREMENTS:
 * Given a number n in the command line, compute and print all the prime numbers less or equal with n.
 * Optimization 3: check only divisors less or equal with sqrt(n).
 * Print the numbers as identified 10 per line.
 * Compute the execution time for the entire program.
 */
public class P263cNaivePrimeOptimization {
    public static void main(String[] args) {
        System.out.println("Naive Prime Optimization (c) by Giovanni Castillo on 09/08/2025");
        ...
        for (int k = 2; k <= n; k++) {
            ...
            for (int i = 2; i <= Math.sqrt(k); i++) { // optimization: check only up to sqrt(k)
                if (k % i == 0) {
                	prime = false;
